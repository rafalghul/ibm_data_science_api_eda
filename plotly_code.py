...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        df1 = filtered_df_new.groupby(['Launch Site', 'class']).size().reset_index(name='class count')
...
def get_scatter_plot(site_dropdown, slider_range):
    ...
    if site_dropdown == 'ALL':
        scatter_figure = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category",
                                    title='Payload vs. Outcome for All Sites')
    else:
        filtered_df1 = filtered_df[filtered_df['Launch Site'] == site_dropdown]
        scatter_figure = px.scatter(filtered_df1, x="Payload Mass (kg)", y="class", color="Booster Version Category",
